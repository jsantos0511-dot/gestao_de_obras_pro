...
# --- 9. TELAS ---
    ...
    if pag == 'RESUMO':
        ...
        if not obras_dict:
            st.info("Nenhuma obra cadastrada. Vá em 'Minhas Obras' para começar.")
        else:
            ...
            if sel:
                ...
                with c2: st.markdown(f'<div class="metric-container"><p class="metric-label">Lucro Prev ({perc_lucro}%)</p><p class="metric-value">{formatar_real(valor_lucro_previsto)}</p></div>', unsafe_allow_html=True)
        ...
        with st.container(border=True):
            ...
            cr = st.text_input("Contato Principal", value=dc["representante"], key=f"cr_{ver}")
        ...
        if obs_dict_l:
            o_f = st.selectbox("Selecione a Obra:", [""] + list(obs_dict_l.keys()), key=f"lo_{ver}")
            if o_f:
                dados = supabase.table("lancamentos_obra").select("*, categorias_obra(nome_categoria), fornecedores(nome_fornecedor)").eq("obra_id", obs_dict_l[o_f]['id']).order("created_at", desc=True).execute().data
                if dados:
                    c_d1, c_d2 = st.columns(2)
                    df_dados = pd.DataFrame(dados)
                    c_d1.download_button("📥 Baixar PDF", gerar_pdf(df_dados, o_f), f"Relatorio_{o_f}.pdf", use_container_width=True)
                    c_d2.download_button("📊 Baixar Excel", gerar_excel(df_dados), f"Relatorio_{o_f}.xlsx", use_container_width=True)
                    for g in dados:
        ...
        with st.container(border=True):
            ne = st.text_input("E-mail do Usuário", key=f"ue_{ver}")
            ...
            if st.button("CRIAR USUÁRIO", use_container_width=True, type="primary"):
                if ne and ns:
                    ...
                    st.success("Usuário criado!"); limpar_campos(); st.rerun()
        
        user_list = supabase.table("usuarios").select("id, email, perfil").execute().data
        if user_list:
