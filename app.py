...
# Funções de PDF e Excel mantidas conforme solicitado
...
# --- 9. TELAS ---
    ...
    if pag == 'RESUMO':
        ...
        if c_sel:
            ...
            if not obs_dict:
                st.warning("Este cliente não possui obras cadastradas.")
            else:
                ...
                if o_sel:
                    ...
                    perc_lucro = float(obra_info.get('lucro_estimado', 0) or 0)
                    perc_imposto = float(obra_info.get('impostos_estimados', 0) or 0)
                    
                    v_lucro_prev = orcado * (perc_lucro / 100)
                    v_imposto = orcado * (perc_imposto / 100)
                    lucro_real = orcado - gasto_total - v_imposto
                    ...
                    with c2: st.markdown(f'<div class="metric-container"><p class="metric-label">Lucro Prev ({perc_lucro}%)</p><p class="metric-value">{formatar_real(v_lucro_prev)}</p></div>', unsafe_allow_html=True)
                    with c3: st.markdown(f'<div class="metric-container"><p class="metric-label">Imposto ({perc_imposto}%)</p><p class="metric-value">{formatar_real(v_imposto)}</p></div>', unsafe_allow_html=True)
                    ...
                    if orcado > 0: st.progress(min(gasto_total / orcado, 1.0))
        ...
        with st.container(border=True):
            ...
            status_p = st.selectbox("Status", ["Pago", "Pendente"], key=f"gs_{ver}")
            foto = st.camera_input("Recibo", key=f"gp_{ver}")
            
            if st.button("SALVAR GASTO", use_container_width=True, type="primary"):
                if c_sel and o_sel and cat_sel and f_sel and v > 0:
                    try:
                        url = None
                        if foto:
                        ...
                        supabase.table("lancamentos_obra").insert({
                            "obra_id": obs_dict[o_sel]['id'], "categoria_id": cats[cat_sel], 
                            "fornecedor_id": forns[f_sel], "descricao": d, "valor": v, 
                            "url_comprovante": url, "status_pagamento": status_p
                        }).execute()
        ...
        # Listagem para edição/exclusão (mantida igual)
        for ob in (supabase.table("obras").select("*").execute().data or []):
            with st.expander(f"🏗️ {ob['nome_obra']}"):
                ...
                if b2.button("Excluir", key=f"dob_{ob['id']}"):
                    supabase.table("lancamentos_obra").delete().eq("obra_id", ob['id']).execute()
                    supabase.table("obras").delete().eq("id", ob['id']).execute(); st.rerun()

    # Telas de CLIENTES, FORNECEDORES, LISTA e USUARIOS permanecem as mesmas para manter a integridade do código
    elif pag == 'CLIE':
        st.markdown("### Clientes")
        dc = {"nome_cliente": "", "representante": "", "telefone": "", "cnpj": ""}
        ...
        with st.container(border=True):
            ...
            if st.button("SALVAR CLIENTE", use_container_width=True, type="primary"):
                p = {"nome_cliente": cn, "representante": cr, "telefone": aplicar_mask_tel(ct), "cnpj": aplicar_mask_cnpj(cc)}
        ...
        for c in (supabase.table("clientes").select("*").order("nome_cliente").execute().data or []):
            with st.expander(f"👤 {c['nome_cliente']}"):
                ...
                if c2.button("🗑️", key=f"dc_{c['id']}"): supabase.table("clientes").delete().eq("id", c['id']).execute(); st.rerun()

    elif pag == 'FORN':
        st.markdown("### Fornecedores")
        df_f = {"nome_fornecedor": "", "telefone": ""}
        ...
        if c_sel:
            ...
            if o_f:
                ...
                if dados:
                    ...
                    c2.download_button("📊 Excel", gerar_excel(df_d), f"{o_f}.xlsx", use_container_width=True)
                    for g in dados:
        ...
        with st.container(border=True):
            ne = st.text_input("E-mail", key=f"ue_{ver}")
            ns = st.text_input("Senha", type="password", key=f"us_{ver}")
            np = st.selectbox("Perfil", ["LANCADOR", "ADMIN"], key=f"up_{ver}")
            if st.button("CRIAR USUÁRIO", use_container_width=True, type="primary"):
                if ne and ns:
                    ...
                    st.success("Criado!"); limpar_campos(); st.rerun()
